 ...
 * 진다. 외부 스토리지 정리(StarImage 원본 파일)는 호출자가 본 port에서 키를 먼저 수집해 외부 스토리지 port로 삭제한 뒤 DB row 삭제를 호출해야 한다.
 *
...
public interface RecordHardDeletePort {
    ...
    /**
     * 사용자가 업로드한 StarImage의 외부 스토리지 키 목록 조회.
     *
     * <p>DB row 삭제 전에 호출되어야 하며, 반환된 키를 외부 스토리지에서 best-effort 삭제 후 {@link
     * #hardDeleteAllByUserId(Long)}를 호출한다. {@code image_key}가 null인 row는 제외(레거시 데이터).
     *
     * @param userId 탈퇴 처리할 사용자 ID
     * @return 외부 스토리지 키 목록 (없으면 빈 리스트)
     */
    List<String> findStarImageKeysByUserId(Long userId);
